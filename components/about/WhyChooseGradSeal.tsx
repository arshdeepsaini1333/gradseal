"use client";

import { motion } from "framer-motion";
import {
  Users,
  BadgeCheck,
  Clock,
  ClipboardCheck,
  IndianRupee,
  Infinity as InfinityIcon,
  MessageSquareQuote,
  Rocket,
  type LucideIcon,
} from "lucide-react";
import { whyChooseFeatures } from "@/lib/about-data";

const iconMap: Record<string, LucideIcon> = {
  Users,
  BadgeCheck,
  Clock,
  ClipboardCheck,
  IndianRupee,
  Infinity: InfinityIcon,
  MessageSquareQuote,
  Rocket,
};

export default function WhyChooseGradSeal() {
  return (
    <section className="py-24 bg-[#F8FAFC]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          {/* Illustration */}
          <motion.div
            initial={{ opacity: 0, x: -40 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, margin: "-80px" }}
            transition={{ duration: 0.7, ease: "easeOut" }}
            className="relative lg:sticky lg:top-28"
          >
            <span className="inline-block px-4 py-1.5 rounded-full bg-[#2563EB]/10 text-[#2563EB] text-sm font-medium mb-4">
              Why GradSeal
            </span>
            <h2 className="text-3xl sm:text-4xl font-extrabold text-[#0F172A] mb-6">
              Everything You Need to{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#2563EB] to-[#06B6D4]">
                Succeed
              </span>
            </h2>
            <p className="text-[#64748B] text-lg leading-relaxed mb-8 max-w-md">
              We built GradSeal from the ground up to give fitness
              professionals the tools, credentials, and confidence to build
              thriving careers.
            </p>

            <motion.div
              animate={{ y: [0, -10, 0] }}
              transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
              className="relative w-full max-w-sm bg-white rounded-3xl shadow-2xl p-8 border border-slate-100"
            >
              <div className="flex items-center gap-3 mb-6">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#2563EB] to-[#06B6D4] flex items-center justify-center text-white text-xl">
                  💪
                </div>
                <div>
                  <p className="font-semibold text-[#0F172A] text-sm">
                    10,000+ Students
                  </p>
                  <p className="text-xs text-[#64748B]">Chose GradSeal</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {["🧘 Yoga", "🏋️ Gym", "🥗 Nutrition", "⚡ Sports"].map((label) => (
                  <div
                    key={label}
                    className="rounded-xl bg-[#F8FAFC] px-3 py-2.5 text-xs font-medium text-[#0F172A] text-center"
                  >
                    {label}
                  </div>
                ))}
              </div>
            </motion.div>
          </motion.div>

          {/* Feature cards */}
          <div className="grid sm:grid-cols-2 gap-5">
            {whyChooseFeatures.map((feature, index) => {
              const Icon = iconMap[feature.icon];
              return (
                <motion.div
                  key={feature.title}
                  initial={{ opacity: 0, y: 28 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true, margin: "-60px" }}
                  transition={{ duration: 0.5, delay: index * 0.07 }}
                  whileHover={{ y: -4 }}
                  className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all"
                >
                  <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-[#EFF6FF] to-[#DBEAFE] flex items-center justify-center mb-4">
                    <Icon className="w-5 h-5 text-[#2563EB]" />
                  </div>
                  <h3 className="font-bold text-[#0F172A] text-base mb-1.5">
                    {feature.title}
                  </h3>
                  <p className="text-[#64748B] text-sm leading-relaxed">
                    {feature.description}
                  </p>
                </motion.div>
              );
            })}
          </div>
        </div>
      </div>
    </section>
  );
}
